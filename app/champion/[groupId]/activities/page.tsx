...
export default function ChampionActivitiesPage() {
  ...
  const [responseMode, setResponseMode] = useState<'feed' | 'direct'>('feed')
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from('activities')
      .select('id, title, description, cta, emoji, season, response_mode, is_published, expires_at, created_at, activity_responses(count)')
      .eq('scope', 'group')
      .eq('group_id', groupId)
  ...
  function resetForm() {
    ...
    setSeason('present'); setResponseMode('feed'); setExpires(''); setIsPublished(true); setFormError('')
  }
    ...
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted text-sm mt-1">Engagement prompts for your group&apos;s members</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="card mb-8 flex flex-col gap-4">
          <h2 className="text-sm font-semibold text-white">New activity (your group)</h2>
          <div>
            <label className="block text-xs text-muted mb-1.5">Title / question *</label>
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="How's training going this week?" className={inputCls} />
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs text-muted mb-1.5">Response goes to</label>
            <select value={responseMode} onChange={e => setResponseMode(e.target.value as 'feed' | 'direct')} className={inputCls}>
              <option value="feed">Member&apos;s Present feed (default)</option>
              <option value="direct">Directly to me (private message)</option>
            </select>
          </div>
          <div>
            ...
            <div key={a.id} className="card flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {a.response_mode === 'direct' && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-white/[0.06] text-muted shrink-0">direct</span>
                  )}
                </div>
